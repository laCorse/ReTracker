...
import torch.backends.cudnn as cudnn # 卷积加速
from torch.optim import lr_scheduler


import models
from util.losses import CrossEntropyLoss, TripletLoss
...
from util.utils import Logger,AverageMeter,save_checkpoint
from util.eval_metrics import evaluate
from util.optimizer import init_optim
import argparse
...
# Datasets
parser.add_argument('--root', type=str, default='/home/qianchen/reid/data', help="root path to data directory")
parser.add_argument('-d', '--dataset', type=str, default='market1501'
                    )
parser.add_argument('-j', '--workers', default=4, type=int,
                    help="number of data loading workers (default: 4)")# 线程数目
...
parser.add_argument('--max-epoch', default=60, type=int,
                    help="maximum epochs to run")# 训练的epoch数目
...
parser.add_argument('--eval-step', type=int, default=-1,
                    help="run evaluation for every N epochs (set to -1 to test after training)")#训练多少个epoch训练一次
...
def main():
    ...
    trainloader = DataLoader(
        ...
        sampler=RandomIdentitySampler(dataset.train, num_instance=args.num_instances),
        batch_size=args.train_batch,
        num_workers=args.workers,
        pin_memory=pin_memory,
        drop_last=True# 训练时会存在epoch % batchsize != 0 的情况，那么余数的图片是否还要训练？drop_last= True就是舍去这些图片
    )
...
def train_easy_version(epoch, model, criterion_class, optimizer, trainloader, use_gpu):
    model.train()
    ...
    for batch_idx, (imgs, pids, _) in enumerate(trainloader):
        ...
        output = model(imgs)
        loss = criterion_class(output, pids)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
...
def train(epoch, model, criterion_class, criterion_metric, optimizer, trainloader, use_gpu):
    ...
    for batch_idx, (imgs, pids, _) in enumerate(trainloader):
        ...
        # 分类损失
        xent_loss = criterion_class(output, pids)
        # Trihard损失
